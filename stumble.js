...
const asciiText = figlet.textSync("VECI VIP", {
    font: 'Graffiti',
    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color(Id, "greyBright"), color(Username), color(Country, "purple"), color(Region, "purpleBright"), color(Crowns, "emeraldBright"), color(SkillRating, "yellowBright")].join(" | ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
